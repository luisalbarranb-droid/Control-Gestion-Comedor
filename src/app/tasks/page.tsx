...
export default function TasksPage() {
  ...
  const tasksQuery = useMemoFirebase(
    () => {
      ...
      // Scope tasks by comedorId
      let baseQuery;
      if (activeComedorId) {
        baseQuery = query(tasksCollection, where('comedorId', '==', activeComedorId));
      } else if (isSuperAdmin) {
        // Global view for SuperAdmins
        baseQuery = tasksCollection;
      } else {
        // Regular users without an active comedor (should not happen)
        return null;
      }
      ...
      if (userId) {
        // Additional filter for regular employees
        return query(baseQuery, where('asignadoA', '==', userId));
      }

      return null;
    },
    [firestore, isAdmin, isSuperAdmin, isAuthLoading, userId, activeComedorId]
  );
  ...
  const usersCollectionRef = useMemoFirebase(
    () => {
      ...
      return (activeComedorId && !isSuperAdmin)
        ? query(ref, where('comedorId', '==', activeComedorId))
        : ref;
    },
  ...
  const handleTaskCreate = (newTaskData: TaskFormValues) => {
    ...
    // Effective comedorId for the task
    const targetComedorId = activeComedorId || currentUser.comedorId;
    if (!targetComedorId) {
      alert("Error: No se ha seleccionado una sede para asignar la tarea.");
      return;
    }
