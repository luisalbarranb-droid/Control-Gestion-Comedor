...
import { SquareCheck, AlertTriangle, ArrowUp, ArrowDown, Minus } from 'lucide-react';
...
function ReportContent() {
  ...
  const { plannedMenu, executedPax, executedItems } = selectedClosing;
    ...
    const processIngredients = (menuItems: (MenuItem[] | DailyClosingItem[]), pax: number, type: 'planned' | 'executed') => {
        for (const item of menuItems) {
            ...
            for (const ingredient of fullMenuItem.ingredients) {
                ...
                let entry = consumptionMap.get(invItem.itemId);
                if (!entry) {
                    entry = { 
                        ingredientId: invItem.itemId, 
                ...
                consumptionMap.set(invItem.itemId, entry);
            }
        }
    };
     ...
     <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex justify-between items-center p-3 bg-muted/50 rounded-md">
                 ...
                 <div>
                    ...
                    {plannedOnlyItems.length > 0 ? (
                        plannedOnlyItems.map(item => <Badge key={item.menuItemId} variant="destructive" className="mr-1">{item.name}</Badge>)
         ...
         <Card>
            ...
            <CardContent>
                <p className="text-sm text-muted-foreground italic">"{selectedClosing.variations}"</p>
            </CardContent>
         </Card>
      </div>
