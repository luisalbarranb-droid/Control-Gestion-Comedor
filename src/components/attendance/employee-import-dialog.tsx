...
const requiredColumns = [
  'name', 'cedula', 'rif', 'email', 'phone', 'address', 'role', 'area', 'workerType', 'contractType',
  'fechaIngreso', 'position', 'diasContrato', 'fechaNacimiento',
  'gender', 'civilStatus', 'nationality',
  'shirtSize', 'pantsSize', 'shoeSize',
  'bloodType', 'allergies', 'diseases',
  'emergencyContactName', 'emergencyContactPhone', 'emergencyContactRelation',
  'bankName', 'bankAccountNumber', 'bankAccountType'
];
...
export function EmployeeImportDialog({ isOpen, onOpenChange, onImport }: EmployeeImportDialogProps) {
  ...
  const parseFile = (fileToParse: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Check if AT LEAST the critical columns are present. We can be lenient with optional fields if we want, 
        // but for now let's enforce all so the template is respected.
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      {
        ...
        phone: '0412-1234567',
        address: 'Av. Principal, Edificio Central',
        role: 'comun',
        area: 'cocina',
        workerType: 'obrero',
        ...
        emergencyContactPhone: '0414-0000000',
      ...
      },
      {
        ...
        phone: '0414-8765432',
        address: 'Calle Secundaria, Casa 5',
        role: 'admin',
        area: 'servicio',
        workerType: 'empleado',
        contractType: 'determinado',
        fechaIngreso: '2024-03-01',
        position: 'Supervisor de Servicio',
        diasContrato: 90,
        ...
        emergencyContactPhone: '0424-0000000',
        emergencyContactRelation: 'Esposo',
        bankName: 'Banesco',
        bankAccountNumber: '01340000000000000000',
    ...
    worksheet["!cols"] = requiredColumns.map(col => ({ wch: 15 })); // Default width for all

    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8' });
    saveAs(data, 'plantilla_empleados_full.xlsx');
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>Importación Masiva de Empleados</DialogTitle>
          <DialogDescription>
            Sube un archivo de Excel para agregar múltiples empleados a la vez.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground pt-2">Valores permitidos para 'role': `comun`, `admin`, `superadmin`. Valores para 'area': `servicio`, `cocina`, `limpieza`, etc. Valores para 'workerType': `obrero`, `empleado`. Valores para 'contractType': `determinado`, `indeterminado`, `prueba`.</p>
          </div>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={handleImportClick} disabled={parsedData.length === 0 || !!error}>
            Importar {parsedData.length > 0 ? `${parsedData.length} Empleados` : ''}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
